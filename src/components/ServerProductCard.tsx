...
export const ServerProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <article className="group bg-white/80 border border-slate-200 rounded-2xl overflow-hidden hover:border-emerald-600/50 transition-all duration-300 hover:shadow-xl hover:shadow-emerald-500/5">
      <Link href={`/products/${product.slug || product.id}`} className="block relative aspect-square overflow-hidden bg-slate-50">
        <Image
          src={product.image}
          alt={product.name}
          fill
        ...
        />
        {discountPercent > 0 && (
          <span className="absolute top-3 left-3 bg-orange-500 text-white font-bold text-[10px] uppercase px-2.5 py-1 rounded-md shadow-md">
            {discountPercent}% OFF
          </span>
        )}
        <span className="absolute bottom-3 right-3 bg-white/90 text-emerald-600 border border-emerald-600/40 text-[10px] font-semibold px-2 py-0.5 rounded-full backdrop-blur-sm">
      ...
      <div className="p-5">
        ...
        <h2 className="font-bold text-base text-slate-900 leading-snug group-hover:text-emerald-600 transition-colors">
          {product.name}
        </h2>
